...
> This is my function and I must be sure that what it only accepts the Bird, or their code will not compile. If anyone wants to use my function, he must be aware that I only accept Bird
> My function get any objects and I just call the object walk() method. So, if the object can walk() it is correct, if it can't my function will fails. So here it is not important the object is a Bird or anything else, it is important that it can walk() (This is duck typing)
